...
class OKCoinSpot:
    ...
    def __init__(self, apikey = '3564fa8c-4afe-4c9b-bef9-7c4a6de66dae', secretkey  = '098F29AB6B3DFB8BFC81421FC33A2A4A'):
        self.__url = 'www.okex.com:443'
    ...
    def get_depth(self,symbol = '',size = ''):
        ...
        params=''
        if symbol:
            params = 'size=%(size)s&symbol=%(symbol)s' %{'size':size, 'symbol':symbol}
    ...
    def limit_order(self,symbol,price,amount,tradeTyp):
        ...
        params['sign'] = buildMySign(params,self.__secretkey)
        res = httpPost(self.__url,TRADE_RESOURCE,params)
        if len(res) > 1:
            return res['order_id']
        else:
            print(res)
